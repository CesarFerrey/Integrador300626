...
describe("Directory", function () {
    ...
    describe("getting parent, child directories, and source files in directory", function () {
        it("should not have a parent loaded initially, but should then load it on request", function () {
            var project = getProject();
            var sourceFile = project.createSourceFile("directory/file.ts");
            var directory = sourceFile.getDirectory();
            chai_1.expect(directory._hasLoadedParent()).to.be.false;
            chai_1.expect(directory.getParent()).to.not.be.undefined;
            chai_1.expect(directory._hasLoadedParent()).to.be.true;
        });
        ...
        it("should have a parent when a file exists in an ancestor folder", function () {
            ...
            testHelpers_1.testDirectoryTree(sourceFile.getDirectory(), {
        ...
        it("should get the child directories", function () {
            ...
            testHelpers_1.testDirectoryTree(file1.getDirectory(), {
    ...
    describe("getParent", function () {
        var project = getProject();
        var sourceFile = project.createSourceFile("/dir/file.ts");
        var dir = sourceFile.getDirectory();
        it("should get the parent when there's a parent", function () {
            chai_1.expect(dir.getParent().getPath()).to.equal("/");
        });
        it("should be undefined for the root directory", function () {
            chai_1.expect(dir.getParentOrThrow().getParent()).to.be.undefined;
        });
    });
    ...
    describe("createSourceFile", function () {
        ...
        it("should note throw an exception if creating a source file at an existing path on the disk and providing the overwrite option", function () {
            var project = getProject([{ filePath: "/file.ts", text: "" }], ["/"]);
            var directory = project.addExistingDirectory("/");
            var fileText = "class Identifier {}";
            var file = directory.createSourceFile("file.ts", fileText, { overwrite: true });
            chai_1.expect(file.getFullText()).to.equal(fileText);
        });
        it("should create a source file with the specified script target", function () {
            var project = getProject([], ["/"]);
            var directory = project.addExistingDirectory("/");
            chai_1.expect(directory.createSourceFile("file.ts", "", { languageVersion: typescript_1.ScriptTarget.ES5 }).getLanguageVersion()).to.equal(typescript_1.ScriptTarget.ES5);
        });
    });
    describe("addExistingSourceFileIfExists", function () {
        it("should return undefined if adding a source file at a non-existent path", function () {
            ...
            var directory = project.createDirectory("dir");
            chai_1.expect(directory.addExistingSourceFileIfExists("non-existent-file.ts")).to.be.undefined;
        });
        it("should add a source file that exists", function () {
            ...
            var sourceFile = directory.addExistingSourceFileIfExists("file.ts");
            chai_1.expect(sourceFile).to.not.be.undefined;
            chai_1.expect(sourceFile.getLanguageVersion()).to.equal(typescript_1.ScriptTarget.Latest);
        });
        it("should add a source file that exists with the specified script target", function () {
            var fileSystem = testHelpers_1.getFileSystemHostWithFiles([{ filePath: "dir/file.ts", text: "" }], ["dir"]);
            var project = new Project_1.Project(undefined, fileSystem);
            var directory = project.addExistingDirectory("dir");
            var sourceFile = directory.addExistingSourceFileIfExists("file.ts", { languageVersion: typescript_1.ScriptTarget.ES5 });
            chai_1.expect(sourceFile).to.not.be.undefined;
    ...
    describe("addExistingSourceFile", function () {
        ...
        it("should add a source file that exists with the specified script target", function () {
            ...
            var project = new Project_1.Project(undefined, fileSystem);
            var directory = project.addExistingDirectory("dir");
            var sourceFile = directory.addExistingSourceFile("file.ts", { languageVersion: typescript_1.ScriptTarget.ES5 });
            chai_1.expect(sourceFile).to.not.be.undefined;
            chai_1.expect(sourceFile.getLanguageVersion()).to.equal(typescript_1.ScriptTarget.ES5);
        });
    });
    describe("createDirectory", function () {
        ...
        it("should have created the directories in the first area", function () {
            testHelpers_1.testDirectoryTree(project.getDirectoryOrThrow("some/path"), {
        ...
        it("should have created the directories in the second area", function () {
            testHelpers_1.testDirectoryTree(project.getDirectoryOrThrow("dir/other"), {
    ...
    describe("addExistingDirectoryIfExists", function () {
        it("should return undefined when the directory doesn't exist", function () {
            ...
            var project = new Project_1.Project(undefined, fileSystem);
            var directory = project.addExistingDirectory("dir");
            chai_1.expect(directory.addExistingDirectoryIfExists("someDir")).to.be.undefined;
        });
        it("should add a directory relative to the specified directory", function () {
            ...
            var project = new Project_1.Project(undefined, fileSystem);
            var directory = project.addExistingDirectory("dir");
            chai_1.expect(directory.addExistingDirectoryIfExists("child")).to.equal(project.getDirectoryOrThrow("dir/child"));
            chai_1.expect(directory.addExistingDirectoryIfExists("../dir2")).to.equal(project.getDirectoryOrThrow("dir2"));
            testHelpers_1.testDirectoryTree(directory.getParentOrThrow(), {
        ...
        it("should add a directory and all its descendant directories when specifying the recursive option", function () {
            ...
            var rootDir = project.addExistingDirectory("/");
            chai_1.expect(rootDir.addExistingDirectoryIfExists("dir", { recursive: true })).to.equal(project.getDirectoryOrThrow("dir"));
            testHelpers_1.testDirectoryTree(project.getDirectoryOrThrow("dir"), {
    ...
    describe("addExistingDirectory", function () {
        ...
        it("should add a directory and all its descendant directories when specifying the recursive option", function () {
            ...
            directories.forEach(function (d) { return project.getFileSystem().mkdirSync(d); });
            var rootDir = project.addExistingDirectory("/");
            chai_1.expect(rootDir.addExistingDirectory("dir", { recursive: true })).to.equal(project.getDirectoryOrThrow("dir"));
            testHelpers_1.testDirectoryTree(project.getDirectoryOrThrow("dir"), {
                directory: project.getDirectoryOrThrow("dir"),
                children: [{
                        directory: project.getDirectoryOrThrow("dir/child1"),
    ...
    describe("copy", function () {
        it("should copy a directory to a new directory", function () {
            ...
            testHelpers_1.testDirectoryTree(newDir, {
                directory: newDir,
        ...
        it("should copy a directory to an existing directory", function () {
            ...
            testHelpers_1.testDirectoryTree(copyDir, {
                directory: copyDir,
                sourceFiles: [project.getSourceFileOrThrow("mainDir/newDir/file.ts")],
                children: [{
