...
describe("StringLiteral", function () {
    ...
    describe("getQuoteKind", function () {
        function doTest(text, quoteKind) {
            var literal = getStringLiteral(text);
            chai_1.expect(literal.getQuoteKind()).to.equal(quoteKind);
        }
        it("should be a double when a double", function () {
            doTest("const t = \"str\";", compiler_1.QuoteKind.Double);
        });
        it("should be a single when a single", function () {
            doTest("const t = 'str';", compiler_1.QuoteKind.Single);
        });
    });
});
