...
describe("Project", function () {
        ...
        it("should set the manipulation settings if provided", function () {
            var project = new Project_1.Project({
                manipulationSettings: {
                    indentationText: options_1.IndentationText.EightSpaces
                }
            });
            chai_1.expect(project.manipulationSettings.getIndentationText()).to.equal(options_1.IndentationText.EightSpaces);
        });
        it("should add the files from tsconfig.json by default with the target in the tsconfig.json", function () {
            ...
            chai_1.expect(project.getSourceFiles().map(function (s) { return s.getLanguageVersion(); })).to.deep.equal([typescript_1.ScriptTarget.ES5, typescript_1.ScriptTarget.ES5]);
        });
    ...
    describe("addExistingDirectoryIfExists", function () {
        it("should throw if the directory doesn't exist", function () {
            ...
            chai_1.expect(project.addExistingDirectoryIfExists("someDir")).to.be.undefined;
        });
        it("should add the directory if it exists", function () {
            var fileSystem = testHelpers.getFileSystemHostWithFiles([], ["someDir"]);
            var project = new Project_1.Project(undefined, fileSystem);
            var dir = project.addExistingDirectoryIfExists("someDir");
            chai_1.expect(dir).to.not.be.undefined;
        });
        it("should add a directory and all its descendant directories when specifying the recursive option", function () {
            var directories = ["/", "dir", "dir/child1", "dir/child2", "dir/child1/grandChild1"];
            var project = new Project_1.Project({ useVirtualFileSystem: true });
            directories.forEach(function (d) { return project.getFileSystem().mkdirSync(d); });
            chai_1.expect(project.addExistingDirectoryIfExists("dir", { recursive: true })).to.equal(project.getDirectoryOrThrow("dir"));
    ...
    describe("addExistingDirectory", function () {
        ...
        it("should add a directory and all its descendant directories when specifying the recursive option", function () {
            ...
            var project = new Project_1.Project({ useVirtualFileSystem: true });
            directories.forEach(function (d) { return project.getFileSystem().mkdirSync(d); });
            chai_1.expect(project.addExistingDirectory("dir", { recursive: true })).to.equal(project.getDirectoryOrThrow("dir"));
            testHelpers.testDirectoryTree(project.getDirectoryOrThrow("dir"), {
                directory: project.getDirectoryOrThrow("dir"),
                children: [{
                        directory: project.getDirectoryOrThrow("dir/child1"),
    ...
    describe("getRootDirectories", function () {
        ...
        it("should add an ancestor dir when requesting it", function () {
            var project = getProject();
        ...
        it("should add the root directory when requesting it", function () {
            var project = getProject();
            chai_1.expect(project.getDirectory("/otherDir")).to.be.undefined;
            project.getDirectoryOrThrow("/");
            chai_1.expect(project.getRootDirectories().map(function (d) { return d.getPath(); })).to.deep.equal([
    ...
    describe("addSourceFilesFromTsConfig", function () {
        ...
        it("should add the files from tsconfig.json with the specified target", function () {
            var fs = new fileSystem_1.VirtualFileSystemHost();
            ...
            chai_1.expect(project.getDirectories().map(function (s) { return s.getPath(); }).sort()).to.deep.equal([].sort());
            ...
            var expectedDirs = ["/test", "/test/test2", "/test/emptyDir"].sort();
            ...
            chai_1.expect(project.getDirectories().map(function (s) { return s.getPath(); }).sort()).to.deep.equal(expectedDirs);
            chai_1.expect(project.getSourceFiles().map(function (s) { return s.getLanguageVersion(); })).to.deep.equal([typescript_1.ScriptTarget.ES5, typescript_1.ScriptTarget.ES5]);
        });
        it("should add the files from tsconfig.json but allow overwriting the target", function () {
            var fs = new fileSystem_1.VirtualFileSystemHost();
            fs.writeFileSync("tsconfig.json", "{ \"compilerOptions\": { \"rootDir\": \"test\", \"target\": \"ES5\" } }");
            fs.writeFileSync("/test/file.ts", "");
            var project = new Project_1.Project({}, fs);
            var returnedFiles = project.addSourceFilesFromTsConfig("tsconfig.json", { languageVersion: typescript_1.ScriptTarget.ES2015 });
            chai_1.expect(project.getSourceFiles().map(function (s) { return s.getLanguageVersion(); })).to.deep.equal([typescript_1.ScriptTarget.ES2015]);
        });
    });
    describe("addExistingSourceFile", function () {
        ...
        it("should add a source file that exists", function () {
            ...
            chai_1.expect(sourceFile.getLanguageVersion()).to.equal(typescript_1.ScriptTarget.Latest);
        });
        it("should add a source file that exists and with the specified target", function () {
            var fileSystem = testHelpers.getFileSystemHostWithFiles([{ filePath: "file.ts", text: "" }]);
            var project = new Project_1.Project(undefined, fileSystem);
            var sourceFile = project.addExistingSourceFile("file.ts", { languageVersion: typescript_1.ScriptTarget.ES5 });
            chai_1.expect(sourceFile).to.not.be.undefined;
            chai_1.expect(sourceFile.getLanguageVersion()).to.equal(typescript_1.ScriptTarget.ES5);
        });
    });
    describe("addExistingSourceFileIfExists", function () {
        it("should return undefined if adding a source file at a non-existent path", function () {
            var project = new Project_1.Project({ useVirtualFileSystem: true });
            chai_1.expect(project.addExistingSourceFileIfExists("non-existent-file.ts")).to.be.undefined;
        });
        it("should add a source file that exists", function () {
            var fileSystem = testHelpers.getFileSystemHostWithFiles([{ filePath: "file.ts", text: "" }]);
            var project = new Project_1.Project(undefined, fileSystem);
            var sourceFile = project.addExistingSourceFileIfExists("file.ts");
            chai_1.expect(sourceFile).to.not.be.undefined;
            chai_1.expect(sourceFile.getLanguageVersion()).to.equal(typescript_1.ScriptTarget.Latest);
        });
        it("should add a source file that exists and with the specified target", function () {
            var fileSystem = testHelpers.getFileSystemHostWithFiles([{ filePath: "file.ts", text: "" }]);
            var project = new Project_1.Project(undefined, fileSystem);
            var sourceFile = project.addExistingSourceFileIfExists("file.ts", { languageVersion: typescript_1.ScriptTarget.ES5 });
            chai_1.expect(sourceFile).to.not.be.undefined;
            chai_1.expect(sourceFile.getLanguageVersion()).to.equal(typescript_1.ScriptTarget.ES5);
        });
    });
    describe("addExistingSourceFiles", function () {
        it("should add based on a string file glob", function () {
            ...
            chai_1.expect(sourceFiles[0].getLanguageVersion()).to.equal(typescript_1.ScriptTarget.Latest);
        ...
        it("should add with the specified target", function () {
            var project = new Project_1.Project({ useVirtualFileSystem: true });
            project.getFileSystem().writeFileSync("file1.ts", "");
            var result = project.addExistingSourceFiles("/**/*.ts", { languageVersion: typescript_1.ScriptTarget.ES5 });
            chai_1.expect(project.getSourceFiles()[0].getLanguageVersion()).to.equal(typescript_1.ScriptTarget.ES5);
        });
        it("should add the directory's descendant directories specified in the glob and ignore negated globs", function () {
            var project = new Project_1.Project({ useVirtualFileSystem: true });
            var fs = project.getFileSystem();
            ["/dir", "/dir2", "/dir/child", "/dir/child/grandChild", "/dir3"].forEach(function (d) { return fs.mkdir(d); });
            var result = project.addExistingSourceFiles(["/dir/**/*.ts", "!/dir2", "/dir3/**/*.ts"]);
            testHelpers.testDirectoryTree(project.getDirectoryOrThrow("/dir"), {
                directory: project.getDirectoryOrThrow("/dir"),
                children: [{
    ...
    describe("createSourceFile", function () {
        ...
        it("should not throw an exception if creating a source file at an existing path when providing the overwrite option", function () {
            var project = new Project_1.Project({ useVirtualFileSystem: true });
            var file1 = project.createSourceFile("file.ts", "");
            var newFileText = "class Identifier {}";
            var file2 = project.createSourceFile("file.ts", newFileText, { overwrite: true });
            chai_1.expect(file1.getFullText()).to.equal(newFileText);
            chai_1.expect(file2.getFullText()).to.equal(newFileText);
            chai_1.expect(file1).to.equal(file2);
        });
        ...
        it("should mark the source file as having not been saved", function () {
            var project = new Project_1.Project({ useVirtualFileSystem: true });
            chai_1.expect(project.createSourceFile("file.ts", "").isSaved()).to.be.false;
        });
        it("should create a source file with the default target", function () {
            var project = new Project_1.Project({ useVirtualFileSystem: true });
            chai_1.expect(project.createSourceFile("file.ts", "").getLanguageVersion()).to.equal(typescript_1.ScriptTarget.Latest);
        });
        it("should allow creating a source file with a specified target", function () {
            var project = new Project_1.Project({ useVirtualFileSystem: true });
            chai_1.expect(project.createSourceFile("file.ts", "", { languageVersion: typescript_1.ScriptTarget.ES5 }).getLanguageVersion()).to.equal(typescript_1.ScriptTarget.ES5);
        });
