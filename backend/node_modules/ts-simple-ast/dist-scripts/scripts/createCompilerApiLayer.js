...
function createCompilerApiLayer(factory) {
    ...
    function createTsSourceFile() {
        ...
        function addSeparatedDeclarations() {
            try {
                for (var _a = __values(allEnums.filter(function (e) { return enumsToSeparate.indexOf(e.getName()) >= 0; })), _b = _a.next(); !_b.done; _b = _a.next()) {
                    ...
                    cloning_1.cloneEnums(sourceFile, [enumDec]).forEach(function (e) { return e.setHasDeclareKeyword(true).setIsExported(false); });
                }
            }
        ...
        function addEnumExports() {
            var filteredEnums = allEnums.filter(function (e) { return enumsToSeparate.indexOf(e.getName()) >= 0; });
            sourceFile.addStatements(function (writer) {
                ...
                for (var i = 0; i < filteredEnums.length; i++) {
                    var enumName = filteredEnums[i].getName();
                    writer.writeLine("tempThis[\"" + enumName + "\"] = tsCompiler." + enumName + ";");
                }
                writer.blankLine();
                writer.write("export ").inlineBlock(function () {
                    for (var i = 0; i < filteredEnums.length; i++) {
                        var enumName = filteredEnums[i].getName();
                        if (i > 0)
